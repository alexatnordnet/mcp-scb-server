 ...
 */
export const PX_WEB_API_REGISTRY = {
 ...
 * Get APIs by country
 */
export function getApisByCountry(country) {
    return Object.values(PX_WEB_API_REGISTRY).filter(api => api.country?.toLowerCase() === country.toLowerCase());
}
/**
 * Get APIs that support a specific language
 */
export function getApisByLanguage(language) {
    return Object.values(PX_WEB_API_REGISTRY).filter(api => api.languages.includes(language.toLowerCase()));
}
/**
...
export function searchApis(query) {
    ...
    return Object.values(PX_WEB_API_REGISTRY).filter(api => api.name.toLowerCase().includes(searchTerm) ||
 ...
 */
export function createCustomApiConfig(id, baseUrl, options = {}) {
    return {
        id,
