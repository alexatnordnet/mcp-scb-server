...
export const ApiSourceSchema = z.object({
    api_source: z.enum(['scb', 'statfi', 'statice', 'hagstova', 'greenland', 'custom'])
 ...
 */
function createClientFromParams(params) {
    if (params.api_source === 'custom') {
        if (!params.custom_endpoint) {
            throw new PxWebError('MISSING_ENDPOINT', 'custom_endpoint is required when api_source is "custom"', 'custom');
        }
...
export async function executeDiscoverApis(params) {
    ...
    // Apply filters
    if (country_filter) {
        apis = apis.filter(api => api.country?.toLowerCase().includes(country_filter.toLowerCase()));
    }
    if (language_filter) {
        apis = apis.filter(api => api.languages.includes(language_filter.toLowerCase()));
    }
    return {
        available_apis: apis,
...
export const getRootNavigationTool = {
...
export async function executeGetRootNavigation(params) {
    ...
    try {
        ...
        throw new PxWebError('NAVIGATION_ERROR', `Failed to get root navigation: ${error instanceof Error ? error.message : 'Unknown error'}`, params.api_source, error);
    }
}
/**
...
export const getNavigationByPathTool = {
...
export async function executeGetNavigationByPath(params) {
    const client = createClientFromParams(params);
    try {
        ...
        throw new PxWebError('NAVIGATION_ERROR', `Failed to navigate to path "${params.path}": ${error instanceof Error ? error.message : 'Unknown error'}`, params.api_source, error);
    }
}
/**
...
export async function executeSearchTables(params) {
    const client = createClientFromParams(params);
    try {
        // Build the endpoint with query parameters
        const queryParams = new URLSearchParams();
        if (params.language)
            queryParams.append('lang', params.language);
        if (params.search_query)
            queryParams.append('query', params.search_query);
        if (params.past_days)
            queryParams.append('pastDays', params.past_days.toString());
        ...
        throw new PxWebError('SEARCH_ERROR', `Failed to search tables: ${error instanceof Error ? error.message : 'Unknown error'}`, params.api_source, error);
    }
}
/**
...
export const getTableMetadataTool = {
...
export async function executeGetTableMetadata(params) {
    const client = createClientFromParams(params);
    try {
        const queryParams = new URLSearchParams();
        if (params.language)
            queryParams.append('lang', params.language);
        if (params.include_default_selection)
            queryParams.append('defaultSelection', 'true');
        ...
        throw new PxWebError('METADATA_ERROR', `Failed to get table metadata: ${error instanceof Error ? error.message : 'Unknown error'}`, params.api_source, error);
    }
}
/**
...
export const queryTableDataTool = {
...
export async function executeQueryTableData(params) {
    const client = createClientFromParams(params);
    try {
        if (params.variable_selections && params.variable_selections.length > 0) {
            ...
            const response = await client.queryData(params.table_id, queryBody, params.format, params.language);
            return response;
        }
        else {
        ...
        throw new PxWebError('QUERY_ERROR', `Failed to query table data: ${error instanceof Error ? error.message : 'Unknown error'}`, params.api_source, error);
    }
}
/**
...
export async function executeGetApiConfig(params) {
    const client = createClientFromParams(params);
    try {
        ...
        throw new PxWebError('CONFIG_ERROR', `Failed to get API configuration: ${error instanceof Error ? error.message : 'Unknown error'}`, params.api_source, error);
    }
}
/**
